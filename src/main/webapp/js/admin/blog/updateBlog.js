...
var updateBlog = function(id) {
	var url = window.location.href;
	var url_param = url.split("=")[1];
	var prarm = '将ID为<span class="text-info">' + url_param + '</span>的成绩<span class="text-success">发表</span>';
	if (id == -1) {
		prarm = '将ID为<span class="text-info">' + url_param + '</span>的成绩放入<span class="text-navy">草稿箱</span>';
	}
	var params = {
		'id' : url_param,
