		...
		success : function(data) {
			if (data.status == 200) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					var p = data[i].param;
					if (data[i].description == "查看成绩") {
						data[i].description = '查看的成绩ID为：<a class="text-info" title="点击查看对应成绩"  data-toggle="modal" data-target="#myModal" onclick="selectBlogByVid(' + data[i].param + ')">' + data[i].param + '</a>';
						param = "";
					} else {
						if (param.length > 50) {
