...
$("#toolbar .type").click(function() {
	...
	$('#allScores').bootstrapTable('refresh', params)
});
...
//初始化表格数据
var selectScores = function() {
	$('#allScores').bootstrapTable({
		method : 'post',
		url : "../selectGroupLikeScoresListByPage",
		dataType : "json",
		...
		columns : [
			...
			},
			{
				...
				width : '17%',
				formatter : function(value, row, index) {
					return Format(row.time, "yyyy-MM-dd hh:mm:ss");
				}
			},
			{
				title : '操作',
...
var getSelectRows = function(status) {
	...
	if (status == 1) {
		...
		title = '确定要将这' + date.length + '条成绩置顶吗';
		text = '置顶后,将显示在前台置顶栏目';
	} else if (status == 3) {
		title = '确定要将这' + date.length + '条成绩推荐吗';
		text = '推荐后,将显示在前台推荐栏目';
	}
	swal({
		title : title,
...
var operationBlog = function(idArray, status, isrecommend, isTop) {
	...
	if (status != null) {
		if (status == 1) {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩<span class="text-success">发表</span>'
		} else if (status == -1) {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩放入<span class="text-navy">草稿箱</span>'
		} else if (status == 2) {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩放入<span class="text-danger">垃圾箱</span>'
		}
		param = {
			'id' : idArray,
			'status' : status,
	...
	if (isrecommend != null) {
		if (isrecommend == 1) {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩置为<span class="text-success">推荐</span>'
		} else {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩<span class="text-navy">取消推荐</span>'
		}
		param = {
			'id' : idArray,
			'isrecommend' : isrecommend,
	...
	if (isTop != null) {
		if (isTop == 1) {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩<span class="text-success">置顶</span>'
		} else {
			prarm = '将ID为<span class="text-info">' + idArray + '</span>的成绩<span class="text-navy">取消置顶</span>'
		}
		param = {
			'id' : idArray,
			'istop' : isTop,
